...
class drawingView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    fun setSizeForBrush(newSize: Int) {
        mBrushSize = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP, newSize.toFloat(), resources.displayMetrics
        ).toInt()
    ...
    fun setEnableDrawing(isEnable: Boolean) {
    ...
    fun saveImage() {
        ...
        try {
            ...
            val gpxfile = File(dir, "mFile.png")

            gpxfile.createNewFile()

            val fileOutputStream= gpxfile.outputStream()

            val bitmap = Bitmap.createBitmap(canvasWidth, canvasHeight, Bitmap.Config.ARGB_8888)
            val canvas = Canvas(bitmap)

            mCanvasBitmap?.compress(Bitmap.CompressFormat.PNG, 100, fileOutputStream)
