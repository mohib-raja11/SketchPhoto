...
public final class GalleryFileSizeHelper {
	...
	public static void mergeBitmapWithPorterMode(Bitmap bitmap, Bitmap bitmap2,
			Mode mode) {
		...
		canvas.drawBitmap(bitmap,
				...
				new Rect(0, 0, bitmap2.getWidth(), bitmap2.getHeight()), paint);
	}

	public static void MakeBitmapForMerge(Context context, Bitmap bitmap,
			int i, Mode mode) throws Throwable {
		Bitmap createBitmap;
		...
		Bitmap decodeStream = BitmapFactory.decodeStream(openRawResource, null,
				options);
		try {
		...
		if (bitmap.getWidth() > bitmap.getHeight()) {
			...
			createBitmap = Bitmap.createBitmap(decodeStream, 0, 0,
		...
		try {
			...
			float max = Math.max(((float) width) / ((float) width2),
					((float) height) / ((float) height2));
			...
			try {
				Rect rect2 = new Rect(0, 0, createBitmap2.getWidth(),
				...
				paint.setAntiAlias(true);
				paint.setFilterBitmap(true);
				canvas2.drawBitmap(createBitmap2, rect2, rect, paint);
	...
	public static Bitmap createScaledBitmapForMerge(Bitmap bitmap, int i) {
		...
		return Bitmap.createScaledBitmap(bitmap,
				(int) Math.floor(((double) width) / d),
				(int) Math.floor(((double) height) / d), true);
	}
	...
	public static Bitmap scaleItBitmap(Bitmap bitmap, double d) {
		...
		return Bitmap.createScaledBitmap(bitmap,
				(int) Math.floor(((double) bitmap.getWidth()) / d),
				(int) Math.floor(((double) bitmap.getHeight()) / d), true);
	}
	...
	public static void DrawRectToBitmap(Bitmap bitmap, float f) {
		...
		Paint paint = new Paint();
		paint.setAntiAlias(true);
		paint.setColor(Color.argb((int) ((((double) f) / 100.0d) * 255.0d),
				MotionEventCompat.ACTION_MASK, MotionEventCompat.ACTION_MASK,
	...
	public static Bitmap getCreatedBitmap(Bitmap bitmap) {
		...
		bitmap.getPixels(mArray, 0, bitmap.getWidth(), 0, 0, bitmap.getWidth(),
				bitmap.getHeight());
		for (int i = 0; i < mArray.length; i++) {
			mArray[i] = 2130706432 | (mArray[i] & 16777215);
		}
		Bitmap createBitmap = Bitmap.createBitmap(bitmap.getWidth(),
				bitmap.getHeight(), Config.ARGB_8888);
		createBitmap.setPixels(mArray, 0, createBitmap.getWidth(), 0, 0,
	...
	public static Bitmap getCreatedBitmapWithMatrix(Bitmap bitmap) {
		...
		GalleryFileSizeHelper.setColorMatrixToBitmap(colorMatrix, 10.0f);
		Paint paint = new Paint();
	...
	public static Bitmap getFilterBitmapMine(Bitmap bitmap, int i) {
		float a = (float) (((((double) i) + 100.0d) / 200.0d) * ((double) GalleryFileSizeHelper
				.setScaleOrColorValue(i)));
		Bitmap createBitmap = Bitmap.createBitmap(bitmap.getWidth(),
				bitmap.getHeight(), Config.ARGB_8888);
